...
class QualityEnhancer:
    ...
    def extract_fov_mask(self, img_bgr, threshold=12):
    ...
    def check_field_of_view(self, mask):
        total_pixels = mask.size
        retinal_pixels = np.count_nonzero(mask)
        return (retinal_pixels / total_pixels) * 100.0

    def compute_quality_metrics(self, img_bgr):
        gray = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2GRAY)
        mask = self.extract_fov_mask(img_bgr)
        fov_coverage = self.check_field_of_view(mask)
    ...
    def assess_quality(self, img_bgr) -> dict:
        """Pipeline adapter method for multi-disease classifier compatibility."""
        focus, illum, decision_score, fov, _ = self.compute_quality_metrics(img_bgr)
        
        reasons = []
        if focus < 35.0:
            reasons.append(f"Out of focus ({focus:.1f} < 35.0)")
        if illum < 25.0:
            reasons.append(f"Poor illumination ({illum:.1f} < 25.0)")
        if fov < 35.0:
            reasons.append(f"Insufficient Retinal FOV ({fov:.1f}% < 35.0%)")

        if len(reasons) > 0:
            status = "Poor"  # Map to Poor / Ungradeable
        elif (focus >= 120.0) and (illum >= 60.0) and (decision_score >= 70.0):
            status = "Good"
        else:
            status = "Borderline"  # Map to Mid quality tier
    ...
    def apply_lab_clahe_enhancement(self, img_bgr, quality_tier="mid"):
    ...
    def enhance_image(self, img_bgr):
        """Pipeline adapter method to enhance borderline quality images."""
        focus, illum, _, _, _ = self.compute_quality_metrics(img_bgr)
        quality_tier = "poor" if (focus < 60.0 or illum < 40.0) else "mid"
        return self.apply_lab_clahe_enhancement(img_bgr, quality_tier=quality_tier)

# --- STANDALONE FUNCTION WRAPPERS FOR BACKWARD COMPATIBILITY ---
_default_enhancer = QualityEnhancer()

def extract_fov_mask(img_bgr, threshold=12):
    return _default_enhancer.extract_fov_mask(img_bgr, threshold)

def check_field_of_view(mask):
    return _default_enhancer.check_field_of_view(mask)

def compute_quality_metrics(img_bgr):
    return _default_enhancer.compute_quality_metrics(img_bgr)

def apply_lab_clahe_enhancement(img_bgr, quality_tier="mid"):
    return _default_enhancer.apply_lab_clahe_enhancement(img_bgr, quality_tier)
...
def process_fundus_image(image_path, output_dir="output_images"):
    ...
    focus, illum, decision_score, fov, mask = _default_enhancer.compute_quality_metrics(img)
    before_metrics = {"Focus": focus, "Illumination": illum, "DecisionScore": decision_score, "FOV": fov}

    reasons = []
    if focus < 35.0: reasons.append(f"Out of focus (Focus score {focus:.1f} < 35.0)")
    if illum < 25.0: reasons.append(f"Poor illumination profile ({illum:.1f} < 25.0)")
    if fov < 35.0: reasons.append(f"Insufficient Retinal FOV ({fov:.1f}% < 35.0%)")

    if len(reasons) > 0:
        return img, None, "Ungradeable", reasons, {"before": before_metrics, "after": None}

    if (focus >= 120.0) and (illum >= 60.0) and (decision_score >= 70.0):
        return img, img, "Good", [], {"before": before_metrics, "after": before_metrics}

    quality_tier = "poor" if (focus < 60.0 or illum < 40.0) else "mid"
    enhanced_img = _default_enhancer.apply_lab_clahe_enhancement(img, quality_tier=quality_tier)

    after_focus, after_illum, after_decision_score, after_fov, _ = _default_enhancer.compute_quality_metrics(enhanced_img)
    after_metrics = {"Focus": after_focus, "Illumination": after_illum, "DecisionScore": after_decision_score, "FOV": after_fov}
...
def resolve_image_path(query):
    if os.path.exists(query): return query
